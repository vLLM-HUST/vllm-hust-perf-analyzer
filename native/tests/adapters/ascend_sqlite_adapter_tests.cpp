...
int main() {
  ...
  require(body_mismatch_candidate.shape_policy ==
          ...
          "single-graph periodic composition retained an H/L/T dependency");
  ...
  require(generic_anchor_stats.device_event_anchors == 7 &&
          ...
          "single-graph exact units did not project to protected anchors");
  std::size_t graph_anchors = 0;
  std::size_t raw_mismatch_anchors = 0;
  for (const AnchorRow& anchor : body_mismatch_ir.anchors.rows()) {
    if (anchor.kind == AnchorKind::kGraphReplayUnit) {
      ++graph_anchors;
      require(body_mismatch_ir.symbols.value(anchor.symbol_id) == "ACLGraph",
              "single-graph exact unit lost its neutral graph symbol");
    } else if (anchor.kind == AnchorKind::kDeviceEvent) {
      ++raw_mismatch_anchors;
    }
  }
  require(graph_anchors == 6 && raw_mismatch_anchors == 1,
          "typed body mismatch did not remain visible beside exact units");
