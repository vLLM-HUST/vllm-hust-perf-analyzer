...
int main() {
  ...
  require(protected_block_result.commit_diagnostics.size() == 2);
  ...
  require(protected_union_result.commit_diagnostics.size() == 2);
  ...
  // Raw-first protected rejection must skip only the repeated-block producer
  // and continue: with A A B x 3 and kNoCross [0,2] the exact outer fold is
  // blocked, but the adjacent AA runs still compress safely.
  std::vector<FixtureProtectedInterval> aa_block_no_cross = {
      FixtureProtectedInterval{ProtectedIntervalKind::kGraphReplayUnit,
                               BoundaryPolicy::kNoCross, 0, 2},
  };
  GlobalGrammarState protected_aa_state =
      make_state({"A", "A", "B", "A", "A", "B", "A", "A", "B"},
                 2, 2, aa_block_no_cross);
  const GrammarEngineResult protected_aa_result =
      run_grammar_state_machine(protected_aa_state);
  require(protected_aa_result.ok());
  require(protected_aa_result.stop_reason == GrammarEngineStopReason::kDone);
  require(protected_aa_state.stage == GrammarStage::kDone);
  require(protected_aa_state.live_node_count == 6);
  require(protected_aa_state.macro_defs.size() == 1);
  require(protected_aa_state.macro_defs[0].level == MacroLevel::kLP);
  require(protected_aa_state.macro_defs[0].rhs_symbols.size() == 2);
  require(protected_aa_state.macro_defs[0].replace_count == 3);
  require(protected_aa_state.nodes[0].macro_def_id == MacroDefId(0));
  require(protected_aa_state.nodes[1].macro_def_id == MacroDefId::invalid());
  require(protected_aa_state.nodes[2].macro_def_id == MacroDefId(0));
  require(protected_aa_state.nodes[3].macro_def_id == MacroDefId::invalid());
  require(protected_aa_state.nodes[4].macro_def_id == MacroDefId(0));
  require(protected_aa_state.nodes[5].macro_def_id == MacroDefId::invalid());
  require(has_action(protected_aa_result.steps,
                     GrammarActionKind::kReplaceExactRuns));
  require(has_action(protected_aa_result.steps,
                     GrammarActionKind::kReplaceRepeatedBlock));
  require(protected_aa_result.commit_diagnostics.size() == 2);
  for (const GrammarCommitDiagnostic& diagnostic :
       protected_aa_result.commit_diagnostics) {
