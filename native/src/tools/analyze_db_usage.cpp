...
void print_usage(const char* argv0) {
  std::cerr << "usage: " << argv0
            ...
            << "Default distributed alignment removes initial rank skew; it is NOT clock calibration.\n"
            << "Use '--distributed-alignment provider' to retain provider timestamps (caller verifies clock comparability).\n"
            << "Views overlap: filter args.projection_plane before summing durations; --no-raw-provider hides audit copies.\n"
...
void print_advanced_usage(const char* argv0) {
  std::cerr << "usage: " << argv0
            << " --source-db <profiler.sqlite-or-db> [--threads N]"
               ...
               " [--distributed-clock-model MODELS.jsonl | --distributed-alignment provider|first-event]"
               " [--no-raw-provider]"
               " [--loop-tree-out PATH|-]"
