...
// protected-boundary rejection returns kBlocked with the step and the
// diagnostics recorded and the state untouched; the caller decides whether to
// skip this producer and continue (raw-first pass) or to stop cleanly at the
// stable pair fixpoint. Any other plan/apply failure is a hard engine error
// (kError). The block step is pushed in the blocked and error paths; callers
// push their own contextual step (for example the pair stop step) before
...
RepeatedBlockOutcome commit_repeated_block_round(
  ...
  if (!plan.valid()) {
    ...
    if (only_protected_boundary_diagnostics(plan.diagnostics)) {
      // Append: a raw-first rejection and a later pair-fixpoint rejection are
      // distinct events, each with a matching action step in the trace.
      result.commit_diagnostics.insert(result.commit_diagnostics.end(),
                                       plan.diagnostics.begin(),
                                       plan.diagnostics.end());
      return RepeatedBlockOutcome::kBlocked;
    }
  ...
  if (!apply.applied()) {
    ...
    if (only_protected_boundary_apply_diagnostics(apply.diagnostics)) {
      result.apply_diagnostics.insert(result.apply_diagnostics.end(),
                                      apply.diagnostics.begin(),
                                      apply.diagnostics.end());
      return RepeatedBlockOutcome::kBlocked;
    }
    state.stage = GrammarStage::kError;
    result.stop_reason = GrammarEngineStopReason::kApplyRejected;
...
GrammarEngineResult run_grammar_state_machine(
  ...
  if (grammar_mode_enables_exact_repeated_block(state.metadata.mode)) {
    ...
    if (block_round.status == GrammarRoundStatus::kActionSelected) {
      ...
      } else if (outcome == RepeatedBlockOutcome::kBlocked) {
        // Protected-boundary rejection at the raw-first pass: skip only this
        // producer and continue, so safe adjacent-run and pair producers can
        // still compress. The step and diagnostics were already recorded and
        // the stage remains kRunFold.
      } else {
        return result;
      }
    } else {
  ...
  while (state.stage == GrammarStage::kPairGrammar) {
    ...
    if (pair_round.status == GrammarRoundStatus::kStop) {
      ...
      // this pass re-applies the same exact whole-sequence check (the block
      // may then be a macro-level block, giving richer nesting than the
      // raw-first pass would recognize). It fires only on exact tilings
      // (block x R, R >= 2) with the same protected-interval validation, so
      // it cannot invent partial or inexact structure.
      if (grammar_mode_enables_exact_repeated_block(state.metadata.mode)) {
        if (++rounds > config.max_rounds) {
          result.stop_reason = GrammarEngineStopReason::kRoundLimitExceeded;
          state.stage = GrammarStage::kError;
        ...
        if (block_round.status == GrammarRoundStatus::kActionSelected) {
          ...
          if (outcome == RepeatedBlockOutcome::kBlocked) {
            // Protected-boundary rejection at the stable pair fixpoint: no
            // other producer can make progress, so stop cleanly with the
