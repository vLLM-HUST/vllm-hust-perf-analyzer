...
std::unordered_map<std::uint32_t, DeviceEventIndex> build_device_event_index(
  ...
  for (const TraceEventRow& event : ir.trace_events.rows()) {
    ...
    if (source.table_name == "ACLGRAPH_REPLAY_UNIT") {
      continue;
    }
...
GraphReplayEvidenceSqlRows build_native_graph_replay_evidence_sql_rows(
  ...
  const auto events_by_device = build_device_event_index(ir);
  ...
  for (const ReplayUnitRow* replay_unit_ptr : replay_units) {
    ...
    replay.graph_event_idx = replay_unit.id.value() + 1;
    ...
    replay.graph_id = template_id(replay_unit.graph_template_id);
    replay.graph_exec_id = replay_id(replay_unit.id);
    ...
    replay.raw_json =
        "{\"reconstruction\":\"capture_stream_task_overlap\","
        "\"graph_template_id\":" +
